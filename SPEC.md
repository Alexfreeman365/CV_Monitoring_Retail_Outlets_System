...
- `journal_mode = WAL` — включается один раз; в `_connect` проверяется на каждом соединении. Читатели и писатель не блокируют друг друга.
...
- 2026-08-16 — Спроектирована SQLite-схема. Приняты решения: `tlt` удалить полностью; `visitor_forecast` добавим в конце; комментарии очистить (цель №7).
