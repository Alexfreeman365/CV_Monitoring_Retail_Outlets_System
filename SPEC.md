...
- `run_cv_task.ps1` проверяет наличие проектного Python, `CV_SYS_v1.py` и модели, устанавливает корень проекта как текущий рабочий каталог и запускает `CV_SYS_v1.py` через `.venv/Scripts/python.exe` с небуферизованным выводом (`-u`); после завершения Python окно остаётся открытым до нажатия `Enter`;
