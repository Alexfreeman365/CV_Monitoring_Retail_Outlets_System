...
3. **Перевести хранение данных с CSV на SQLite** (вся система сейчас работает на CSV-файлах в папке `db/`).
4. Устранить технический долг (см. раздел 11), не ломая работу системы.
5. **Унифицировать текстовый интерфейс всех модулей**: первый запуск exe создаёт файл-запрос с описанием и параметрами (программа завершается), повторный запуск читает сохранённые параметры и стартует. Привести к этому все текстовые модули.
6. **Очистить комментарии в коде**: ~~убрать простые/очевидные, оставить только в сложных местах~~; единый язык комментариев — английский. **Выполнено 2026-08-16 (язык):** все русские комментарии переведены на английский. Удаление очевидных комментариев — опционально, не выполнялось.
...
Примечания к схеме:
- `visitor_forecast` — прогноз посетителей, генерируется отдельным модулем (Prophet, отдельная среда `.venv-forecast`), см. раздел 6.6.
...
> **Ограничение (на текущий момент):** система корректно работает при обработке **одного** нового дня за запуск (штатный ежедневный режим). Оркестровка идёт по камерам, а `vis_count_noseller_pipeline` запускается на границе дней внутри `shape_detection` основной камеры; при накоплении **нескольких** необработанных дней подряд вторая камера группы (`chm2`) на момент запуска алгоритма может быть ещё не обсчитана. Зафиксировано как текущее ограничение — не исправлялось, чтобы не менять проверенную годами логику.
...
**Среда.** `.venv-forecast` (Windows) — `pandas`, `numpy`, `prophet` (тянет `cmdstanpy`). В главной среде Prophet не ставится. `holidays` не используется (январские праздники маскируются NaN). Запуск: `.venv-forecast\Scripts\python.exe utils/funcs_visitor_forecast.py <cwd>`.
...
12. **`visitor_forecast`** — ~~генерировался отдельным модулем вручную~~ **Решено 2026-08-19:** модуль `utils/funcs_visitor_forecast.py` (Prophet) в отдельной среде `.venv-forecast`; таблица SQLite `visitor_forecast`, выгрузка `db/visitor_forecast.csv` для дашборда, гейт + subprocess-запуск из `vis_count_noseller_pipeline`.
